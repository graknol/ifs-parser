...
PROCEDURE Clear_Completed_Date___ (
   attr_ IN OUT VARCHAR2,
   rec_  IN     activity_tab%ROWTYPE)
IS
   objid_       VARCHAR2(50);
   objversion_  VARCHAR2(260);
   newrec_      activity_tab%ROWTYPE;
   oldrec_      activity_tab%ROWTYPE;
   indrec_      Indicator_Rec;
BEGIN
   newrec_ := Lock_By_Keys___(rec_.activity_seq);
   newrec_.completed_date := NULL;
   newrec_.actual_finish := NULL;
   oldrec_ := newrec_;
   Get_Id_Version_By_Keys___(objid_, objversion_, newrec_.activity_seq);
   Unpack___(newrec_, indrec_, attr_);
   ...
   Client_SYS.Add_To_Attr('COMPLETED_DATE',newrec_.completed_date, attr_);
END Clear_Completed_Date___;


PROCEDURE Validate_Fin_Responsible___ (
   company_         IN VARCHAR2,
   fin_responsible_ IN VARCHAR2 )
IS
   dummy_   NUMBER;
   
   CURSOR get_user_cur IS
      SELECT 1
      FROM   person_info
      WHERE  user_id IN (SELECT userid
                         FROM   user_finance
                         WHERE  company   = company_)
                           AND  person_id = fin_responsible_;
BEGIN
   OPEN  get_user_cur;
   FETCH get_user_cur INTO dummy_;
   IF get_user_cur%NOTFOUND THEN
      CLOSE get_user_cur;
      Error_SYS.Record_General(lu_name_, 'INVALIDFINRES: Financially Responsible is not in the Company of the Project to which this Activity is connected!');
   END IF;
   CLOSE get_user_cur;
END Validate_Fin_Responsible___;


PROCEDURE Check_Activity___ (
   rec_ IN activity_tab%ROWTYPE )
IS
   check_connected_    NUMBER;
BEGIN
   $IF (Component_Order_SYS.INSTALLED) $THEN
      check_connected_ := Customer_Order_API.Check_Order_Connected(Get_Project_Id(rec_.activity_seq), rec_.activity_seq);
      IF (check_connected_ = 1) THEN
         Error_SYS.Record_General('Activity', 'CUSTORDCON: Activity sequence :P1 is connected to a customer order. Operation not allowed.', rec_.activity_seq);
      END IF;
   $ELSE
      NULL;
   $END
END Check_Activity___;


FUNCTION Check_Promote_To_Released___ (
   rec_ IN activity_tab%ROWTYPE ) RETURN BOOLEAN
IS
BEGIN
   IF (Release_Allowed___ (rec_.project_id)) THEN
      RETURN TRUE;
   ELSE
      RETURN FALSE;
   END IF;
END Check_Promote_To_Released___;


FUNCTION Check_Promote_To_Cancel___ (
   rec_ IN activity_tab%ROWTYPE) RETURN BOOLEAN
IS
   contract_project_          VARCHAR2(6);
   contract_item_             VARCHAR2(6);
   contract_activity_         VARCHAR2(6);
   company_                   VARCHAR2(20);
   project_id_                VARCHAR2(50) := NULL;
   accounting_year_     NUMBER;
   accounting_period_   NUMBER;
BEGIN
   -- Check whether the inventory on hand exists for the activity
   $IF (Component_Invent_SYS.INSTALLED) $THEN
      IF (Inventory_Part_In_Stock_API.Activity_Has_Parts_In_Stock(rec_.activity_seq)) THEN
         Error_SYS.Record_General(lu_name_, 'CANTCANCELACT: Activity cannot be cancelled since inventory on hand exists.');
      END IF;
   $END
   
   $IF (Component_Invoic_SYS.INSTALLED) $THEN
      IF (Invoice_Utility_Pub_API.Is_Activity_In_Use(rec_.activity_seq) = 1) THEN
         Error_SYS.Record_General(lu_name_, 'ACTCONNINVOICE2: The activity is connected to an invoice. Activity can not be cancelled!', NULL, NULL, NULL);
      END IF;
   $END
   
   IF (Check_Allocations_Exists(rec_.activity_seq) = 'TRUE') THEN
      Error_SYS.Record_General(lu_name_, 'ACTRESALLOEXIST: Unable to cancel activity when resource allocations exist. Please remove these first!');
   END IF;
   
   IF (Project_API.Get_Objstate(rec_.project_id) = 'Approved') THEN
      IF ((NVL(Project_Connection_Temp_API.Get_Baseline_Cost(rec_.activity_seq), 0) != 0) OR (NVL(Project_Connection_Temp_API.Get_Baseline_Hours(rec_.activity_seq), 0) != 0)) THEN
         Error_SYS.Record_General(lu_name_, 'NOTPOSTOCANCELBASE: Not possible to cancel an activity with a baseline', NULL, NULL, NULL);
      END IF;
   END IF;
   
   $IF (Component_Conmgt_SYS.INSTALLED) $THEN
      contract_project_  := Contract_Project_API.Check_Activity_Exist(rec_.activity_seq);
      contract_item_     := Contract_Item_API.Check_Activity_Exist(rec_.activity_seq);
      contract_activity_ := Contract_Activity_API.Check_Activity_Exist(rec_.activity_seq);
      IF ((contract_project_ = 'TRUE') OR (contract_item_ = 'TRUE') OR (contract_activity_ = 'TRUE')) THEN
         Error_SYS.Record_General(lu_name_, 'CANTCANCEL: Connection to sales contract exist - remove sales contract connection before cancelling', NULL, NULL, NULL);
      END IF;
   $END
   
   $IF (Component_Prjdel_SYS.INSTALLED) $THEN
      IF (Planning_Item_API.Check_Activity_Exists(rec_.activity_seq)) THEN
         Error_SYS.Record_General(lu_name_, 'CANTCANCELPLANNINGITEMS: Activity :P1 (Activity sequence :P2) has connected project deliverables planning items, remove planning items before cancelling', Get_Activity_No(rec_.activity_seq), rec_.activity_seq, NULL);
      END IF;
   $END
   
   IF (Project_Quotation_List_API.Check_Exist_Pql_For_Activity(rec_.activity_seq) = 'TRUE') THEN
      Error_SYS.Record_General(lu_name_, 'CANTCANPQL: Project quotation line exist in the PQL - remove line before cancelling', NULL, NULL, NULL);
   END IF;
   
   IF (Check_Exist_Expense_Object(rec_.activity_seq) = 'TRUE') THEN
      Error_SYS.Record_General(lu_name_, 'CANTCANCELEXPOBJ: Expense sheet(s) connected to activity should be removed before cancelling activity.');
   END IF;
   
   Check_Activity___(rec_);
   
   IF (Project_Connection_API.Check_Exist_Non_Removables(rec_.activity_seq) = 'TRUE') THEN
      Error_SYS.Record_General(lu_name_, 'CANTCANOBJ: Only the following object types: Resource, Dependency, Task, PR Line, No Part PR Line, Estimate, Misc Demand, Act Requirement may be cancelled. Manually remove other object types for activity :P1 (Activity sequence :P2) before cancelling.', Get_Activity_No(rec_.activity_seq), rec_.activity_seq, NULL);
   END IF;
   
   -- Check posting transferred    
   $IF (Component_Mpccom_SYS.INSTALLED) $THEN
      company_ := Project_API.Get_Company(Get_Project_Id(rec_.activity_seq));
      IF (Mpccom_Accounting_API.Activity_Postings_Transferred(company_, rec_.activity_seq) = 'FALSE') THEN
         Error_SYS.Record_General(lu_name_, 'OBJSSNOTTRSERR: There exists transactions that is posted on the activity :P1 (Activity Sequence :P2) and which has not been transferred to financials.', rec_.activity_no, rec_.activity_seq);
      END IF;
   $END      
   
   Project_Accrual_Item_API.Validate_Activity(project_id_, accounting_year_, accounting_period_, rec_.activity_seq);
   IF project_id_ IS NOT NULL THEN
      Error_SYS.Record_General(lu_name_, 'VOUCHERNOTCREATED: Activity :P1 is included in Project Accrual for Project :P2, Accounting Period :P3 .',rec_.activity_seq ,project_id_, accounting_year_ || '-' || accounting_period_ );
          
   END IF;
      
   RETURN TRUE;
END Check_Promote_To_Cancel___;


FUNCTION Check_Demote_To_Planned___ (
   rec_ IN activity_tab%ROWTYPE ) RETURN BOOLEAN
IS
   nexist_         NUMBER;
   activity_seq_   NUMBER;
   temp_           NUMBER;
   accounting_currency_code_ VARCHAR2(3);

   CURSOR check_conn_objects IS
      SELECT 1
      FROM project_connection_tab
      WHERE activity_seq = activity_seq_
        AND ((proj_lu_name IN ('AT','AR','PRJ') AND (NVL(Project_Connection_Details_API.Get_Progress_Cost(activity_seq, keyref1, keyref2, keyref3, keyref4, keyref5, keyref6, proj_lu_name), 0) > 0 OR 
                                                     NVL(Project_Connection_Details_API.Get_Progress_Hours(activity_seq, keyref1, keyref2, keyref3, keyref4, keyref5, keyref6, proj_lu_name), 0) > 0)) OR
             proj_lu_name NOT IN ('AT','PRJ','AR','AE'));

   CURSOR activity_connections(activity_seq_ NUMBER) IS
      SELECT DECODE(keyref3,'*',NULL,keyref3) keyref3, decode(keyref4,'*',NULL,keyref4) keyref4, proj_lu_name
      FROM   project_connection_tab
      WHERE  activity_seq = activity_seq_
      AND   (proj_lu_name = 'PT' OR proj_lu_name = 'PTR');
BEGIN
   activity_seq_ := rec_.activity_seq;
   -- Check IF no hours registered!
   -- Note: keyref3 and keyref4 contains account date or currency code depending on proj_lu_name
   $IF (Component_Prjrep_SYS.INSTALLED) $THEN
      accounting_currency_code_ := Company_Finance_API.Get_Currency_Code(Project_API.Get_Company(rec_.project_id));
      nexist_ := 0;
      FOR actconrec_ IN activity_connections(activity_seq_) LOOP
         IF actconrec_.proj_lu_name = 'PT' THEN
            IF Project_Transaction_API.Has_Activity_Transactions(activity_seq_, accounting_currency_code_, to_date(actconrec_.keyref3,'YYYYMMDD')) = '1' THEN
               nexist_ := 1;
               EXIT;
            END IF;
         ELSE
            IF Project_Transaction_API.Has_Activity_Transactions(activity_seq_, actconrec_.keyref3, to_date(actconrec_.keyref4,'YYYYMMDD')) = '1' THEN
               nexist_ := 1;
